...
def test_f1(source, destination, check_source):
    return 1,1
    #g.vertices()
...
def parallel_pagerank(iterations, graph):
    ...
    #test_links = hyperedge_links.map(lambda el: (el[0],hyperedge_links.flatMap(lambda x:test_f1(x[0],x[1],el[1]))))
    test_links = hyperedge_links.map(lambda el: (el[0],1))
